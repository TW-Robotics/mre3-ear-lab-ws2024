...
int main( int argc, char** argv )
{
    if(argc != 6)
    {
        std::cout << FORERED << "Not enough arguments (map, ymal, gauss_out, gauss_resulution, draw_header(0 = false,1 = true), µSv/h(0 = false,1 = true )" << std::endl;
        return -1;
    }
    ...
    //read gauss csv
    std::vector<Gauss> pos_gauss2;
    read_csv_gauss(pos_gauss2, argv[3], map.rows, resulution, cv::Point2f(origin[0], origin[1]) );

    draw_radiolocial( map, pos_gauss2, resulution, (gauss_resulution/resulution), argv[6] );

    //cv::namedWindow("Result", CV_WINDOW_AUTOSIZE);
    //cv::imshow("Result", map);

	cv::imwrite( path + "/map_gauss.ppm", map );
